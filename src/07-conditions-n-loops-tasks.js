...
function isCreditCardNumber(ccn) {
  const summary = [...String(ccn)]
    ...
    .map((num, i) => {
      ...
      while (current > 9) {
        current = [...String(current)].reduce((sum, dig) => sum + Number(dig), 0);
      }
      return Number(current);
    })
    .reduce((sum, num) => sum + num, 0);
  return (10 - (summary % 10)) % 10 === Number(String(ccn).slice(-1));
}
...
function getDigitalRoot(num) {
  const sum = [...String(num)].reduce((acc, el) => acc + Number(el), 0);
